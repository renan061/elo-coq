...
Ltac inversion_not_access :=
  match goal with
    | [ H : ~ access _ (TM_New _) _ |- _ ] =>
        eapply inversion_not_access_new in H; clear H
    | [ H : ~ access _ (TM_Load _) _ |- _ ] =>
        eapply inversion_not_access_load in H; clear H
    | [ H : ~ access _ (TM_Asg _ _) _ |- _ ] =>
        eapply inversion_not_access_asg in H; clear H
    | [ H : ~ access _ (TM_Seq _ _) _ |- _ ] =>
        eapply inversion_not_access_seq in H; clear H
  end.

Lemma access_load_not : forall m t ad,
  ~ access m t ad ->
  ~ access m (TM_Load t) ad.
Proof.
...
Lemma access_asg_not : forall m l r ad,
  ~ access m l ad /\ ~ access m r ad ->
  ~ access m (TM_Asg l r) ad.
Proof.
  intros * [? ?] ?. inversion_access; eauto.
...
Lemma access_seq_not : forall m t1 t2 ad,
  ~ access m t1 ad /\ ~ access m t2 ad ->
  ~ access m (TM_Seq t1 t2) ad.
Proof.
  intros * [? ?] ?. inversion_access; eauto.
