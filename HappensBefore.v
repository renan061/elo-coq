...
Inductive happens_before : trace -> Prop :=
  | hb_thread : forall tc tid e1 e2,
    happens_before <((tid, e1) -{tc}- (tid, e2))>
  ...
  | hb_initialize_acquire : forall tc tid1 tid2 ad t e,
    is_acquire ad e ->
    happens_before <((tid1, e_init ad t) -{tc}- (tid2, e))>

  | hb_transitivity : forall tcA tcB ev1 ev2 ev3,
    happens_before <(ev1 -{tcA              }- ev2)> ->
    happens_before <(ev2 -{tcB              }- ev3)> ->
    happens_before <(ev1 -{tcB ++ ev2 :: tcA}- ev3)>
  .

(* lemmas ------------------------------------------------------------------ *)

Local Lemma happens_before_from_initialize_then_acquire :
  forall tc tc1 tc2 tc3 tid1 tid2 e1 e2 eAcq ad t,
    is_acquire ad eAcq  ->
    tc = (tc3 ++ (tid2, eAcq) :: tc2 ++ (tid1, e_init ad t) :: tc1) ->
    happens_before <((tid1, e1) -{tc}- (tid2, e2))>.
Proof.
  intros * ? Htc.
  rewrite app_comm_cons in Htc. rewrite app_assoc in Htc.
  remember (tc3 ++ (tid2, eAcq) :: tc2) as tc' eqn: Htc'.
  rewrite Htc.  eapply hb_transitivity; eauto using happens_before.
  rewrite Htc'. eapply hb_transitivity; eauto using happens_before.
Qed.

Local Lemma happens_before_from_release_then_acquire :
...
Proof.
  ...
  eauto using happens_before_from_initialize_then_acquire.
Qed.
...
Proof.
  ...
  eauto using happens_before_from_release_then_acquire.
Qed.
