...
Proof.
  ...
  injection H1 as Hnil Hcons.
  rewrite Hnil. clear Hnil.
  rewrite H2 in Hcons. clear H2.
  injection Hcons as H.
  symmetry. apply H.
Qed.
(** [] *)
...
Proof.
  ...
  - destruct m.
    + intros H. discriminate H.
    + intros H. f_equal. rewrite <- IHn.
      * reflexivity.
      * destruct eqb. reflexivity. discriminate H.
Qed.
(** [] *)
...
Proof.
  intros n. induction n as [| n IHn].
    ...
    - destruct m. intros H. discriminate H.
      + intros H. f_equal.
        assert (Hs : forall x y, S x + S y = S (S (x + y))). {
          intros x y. rewrite <- (plus_n_Sm (S x) y). rewrite add_comm.
          rewrite <- plus_n_Sm. rewrite add_comm. reflexivity.
        }
        rewrite 2 Hs in H. injection H. intros Hnm. apply IHn. apply Hnm.
Qed. 
(** [] *)
...
Theorem f_equal' {A B : Type} : forall (f: A -> B) (x y: A),
  x = y -> f x = f y.
Proof. intros f x y H. rewrite H. reflexivity.  Qed.

Theorem nth_error_after_last: forall (n : nat) (X : Type) (l : list X),
  length l = n ->
  nth_error l n = None.
Proof.
  intros n X l H. generalize dependent n. induction l as [| x xs IHl].
  - reflexivity.
  - simpl. destruct n.
    + intros H. discriminate H.
    + intros H.
...
Theorem combine_split : forall X Y (l : list (X * Y)) l1 l2,
  ...
  combine l1 l2 = l.
Proof.
  intros X Y l. induction l as [| x xs IHl].
  - intros l1 l2 H. simpl. injection H as H1 H2.
    rewrite <- H1, <- H2. reflexivity.
  - destruct x as [x1 x2]. simpl.
    destruct (split xs) as [xs1 xs2] eqn:Eqxs.
    intros l1 l2 H. injection H as H1 H2.
    rewrite <- H1, <- H2. simpl. f_equal.
    apply (IHl xs1 xs2).
    reflexivity.
Qed.
...
Proof.
  intros n m p H1 H2.
  apply (eqb_true n m) in H1.
  apply (eqb_true m p) in H2.
  rewrite H1, H2, eqb_refl. reflexivity.
Qed.
(** [] *)
...
Lemma Sn_0 : forall n,
  S n = 0 -> False.
Proof.
  intros n H. destruct n; discriminate.
Qed.

Lemma length_0_nil : forall X (l : list X),
  length l = 0 -> l = [].
Proof.
  intros X l H. destruct l.
  - reflexivity.
  - simpl in H. apply Sn_0 in H. contradiction.
Qed.

Theorem split_combine : split_combine_statement.
Proof.
  intros X Y l1. induction l1 as [| x xs IH].
  - simpl. intros l2 H. symmetry in H.
    apply length_0_nil in H. rewrite H. reflexivity.
  - simpl. destruct l2 as [| y ys]; simpl; intros H.
    + apply Sn_0 in H. contradiction.
    + apply (eq_add_S (length xs) (length ys)) in H.
      rewrite (IH ys H). reflexivity.
Qed.
...
Theorem filter_exercise : forall (X : Type) (test : X -> bool)
  ...
  test x = true.
Proof.
  intros X test x l lf. induction l as [| b bs IH].
  - simpl. discriminate.
  - simpl. destruct (test b) eqn:E.
    + intros H. inversion H. rewrite <- H1, E. reflexivity.
    + intros H. rewrite H in IH. rewrite IH; reflexivity.
Qed.
...
Proof.
  intros X test l. induction l as [| x xs IH].
  - reflexivity.
  - unfold existsb'. simpl. destruct (test x).
    + reflexivity.
    + simpl. unfold existsb' in IH. apply IH.
Qed.
