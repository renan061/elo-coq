...
Proof.
  intros n m H. destruct n, m; split; try reflexivity; try discriminate.
Qed.
(** [] *)
...
Theorem contrapositive : forall (P Q : Prop),
  (P -> Q) -> (~Q -> ~P).
Proof.
  intros P Q H1 H2. unfold not. intros HP. apply H1 in HP.
  unfold not in H2. apply H2 in HP. apply HP.
Qed.
(** [] *)
...
Theorem not_both_true_and_false : forall P : Prop,
  ~ (P /\ ~P).
Proof.
  intros P [HP HnP]. unfold not in HnP. apply HnP in HP. apply HP.
Qed.
(** [] *)
...
Proof.
  intros X P H1 [x H2]. unfold not in H2. apply H2. apply H1.
Qed.
(** [] *)
...
Theorem In_map_iff :
  forall (A B : Type) (f : A -> B) (l : list A) (b : B),
         In b (map f l) <->
         exists a, f a = b /\ In a l.
Proof.
  intros A B f l b. induction l as [| x xs IH]; simpl; split.
  - contradiction.
  - simpl. intros [a F]. destruct F as [_ F]. contradiction.
  - intros [H | H].
    + exists x. split.
      * apply H.
      * left. reflexivity.
    + apply IH in H. destruct H as [a [Hl Hr]]. exists a. split.
      * apply Hl.
      * right. apply Hr.
  - intros [a [Hl [Hr1 | Hr2]]].
    + rewrite <- Hr1 in Hl. left. apply Hl.
    + right. apply IH. exists a. split.
      * apply Hl.
      * apply Hr2.
Qed.
(** [] *)
...
Proof.
  intros A l l2. induction l as [| a' l' IH]; split.
...
Proof.
  intros T P l. induction l as [| x xs IH]; split.
  - simpl. intros H. reflexivity.
  - simpl. intros H x F. contradiction.
  - simpl. intros H. split.
    + apply H. left. reflexivity.
    + apply IH. intros x' H'. apply H. right. apply H'.
  - simpl. intros [Hl Hr] x' [H | H].
    + rewrite <- H. apply Hl.
    + apply IH.
      * apply Hr.
      * apply H.
...
Proof.
  intros Po Pe n H1 H2. induction n as [| n IH].
  - unfold combine_odd_even. simpl. apply H2. unfold odd. reflexivity.
  - unfold combine_odd_even. simpl. destruct n.
    + apply H1. unfold odd. reflexivity.
    + unfold odd in H1. simpl in H1.
      unfold odd in H2. simpl in H2.
      unfold odd in IH. simpl in IH. destruct n in IH.
      * destruct (even n).
...
Proof.
  ...
  - unfold odd. rewrite E. discriminate.
  - intros H _. apply H.
Qed.
...
(* unused *)
Lemma l_eq_revl : forall X (l : list X) e,
  l = [] \/ l = [e] ->
  l = rev l.
Proof.
  intros X l e. induction l as [| x xs IH].
  - reflexivity.
  - simpl. intros [H | H].
    + discriminate H.
    + injection H as H1 H2. rewrite H1, H2. reflexivity.
Qed.
...
Lemma negb_eq : forall b1 b2, negb b1 = b2 -> b1 = negb b2.
Proof.
  destruct b1, b2; intros H; try discriminate H; reflexivity.
Qed.
...
Proof.
  ...
  - rewrite even_S. destruct (negb (even n)) eqn:E.
    + apply negb_eq in E. simpl in E. rewrite E in IH.
      rewrite IH. exists (S(k)). reflexivity.
    + apply negb_eq in E. simpl in E. rewrite E in IH.
      rewrite IH. exists k. reflexivity.
Qed.
(** [] *)
...
Proof.
  split; destruct b1, b2; intros H.
  - split; reflexivity.
  - unfold andb in H. discriminate H.
  - unfold andb in H. discriminate H.
  - unfold andb in H. discriminate H.
  - unfold andb. reflexivity.
...
Theorem orb_true_iff : forall b1 b2,
  b1 || b2 = true <-> b1 = true \/ b2 = true.
Proof.
  destruct b1, b2; split.
  - left. reflexivity.
  - unfold orb. reflexivity.
  - left. reflexivity.
  - unfold orb. reflexivity.
  - right. reflexivity.
  - unfold orb. reflexivity.
  - intros H. unfold orb in H. discriminate H.
...
Lemma aux : forall x y,
  S x <> S y -> x <> y.
Proof.
  intros x y H. destruct x, y.
  + contradiction.
  + unfold not. intros J. rewrite J in H. contradiction.
  + unfold not. intros J. rewrite J in H. contradiction.
  + unfold not. intros J. rewrite J in H. contradiction.
Qed.
...
Theorem eqb_neq : forall x y : nat,
  x =? y = false <-> x <> y.
Proof.
  induction x; induction y; split; intros H.
  - discriminate H.
  - contradiction.
  - unfold not. intros I. discriminate I.
  - unfold eqb. reflexivity.
  - unfold not. intros I. discriminate I.
  - unfold eqb. reflexivity.
  - unfold not. intros I. rewrite I in H.
    simpl in H. rewrite eqb_refl in H. discriminate H.
  - simpl. apply aux in H. apply IHx. apply H.
Qed.
(** [] *)
...
Lemma aux1_eqb_list_true_iff : forall {X} (x : X) l1 l2,
  x :: l1 = x :: l2 ->
  l1 = l2.
Proof.
  intros X x l1 l2 H. injection H. intros J. apply J.
Qed.
...
Proof.
  intros A eqb H. split; generalize dependent l2.
  - induction l1 as [| x xs IHx].
    + destruct l2. reflexivity. intros F. discriminate F.
    + simpl. destruct l2 as [| y ys].
      * intros F. discriminate F.
      * destruct (eqb x y) eqn:E.
        { intros J. apply H in E. rewrite E. f_equal. apply IHx. apply J. }
        { intros F. discriminate F. }
  - induction l1 as [| x xs IHx].
    + destruct l2. reflexivity. intros F. discriminate F.
    + simpl. destruct l2 as [| y ys]. 
      * intros F. discriminate F.
      * destruct (eqb x y) eqn:E.
        { apply H in E. intros J. apply IHx. rewrite E in J.
          apply aux1_eqb_list_true_iff in J. apply J. }
        { intros J. injection J as H1 H2. rewrite <- E.
          apply H. apply H1. }
Qed. 
...
Proof.
  ...
  - induction l as [| x xs IH].
    ...
    + simpl. intros H. split.
      ...
      * apply IH. apply andb_true_elim2 in H. apply H.
  - induction l as [| x xs IH].
    + reflexivity.
    + simpl. intros [Hl Hr]. apply IH in Hr.
