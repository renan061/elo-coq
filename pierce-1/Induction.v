...
Proof.
  intros n m p.
  rewrite add_comm.
  assert (H1 : n + p = p + n). { rewrite add_comm. reflexivity. }
  rewrite H1. rewrite add_assoc. reflexivity.
  Qed.
...
Theorem help : forall x k,
  x * S k = x + x * k.
Proof.
  intros x k. induction x as [|x IHx].
  - reflexivity.
  - simpl. rewrite add_assoc.
    assert (H : k + x = x + k). { rewrite add_comm. reflexivity. }
    rewrite <- H. rewrite IHx. rewrite add_assoc. reflexivity.
  Qed.
...
Proof.
  ...
  - simpl. rewrite help. rewrite <- IHn. reflexivity.
  Qed.  
...
Proof.
  intros n m p.
  assert (helper : n <=? m = ((p + n) <=? (p + m))). {
    induction p as [|p IHp].
    - reflexivity.
    - simpl. rewrite IHp. reflexivity.
  }
  rewrite helper. intros H. rewrite H. reflexivity.
  Qed.
...
Proof.
  ...
  - rewrite (help (n + m) p).
    ...
    assert (I : n + m + n * p = n + n * p + m).
    + rewrite <- (add_assoc n m (n*p)).
      rewrite (add_comm m (n*p)).
      rewrite add_assoc.
      reflexivity.        
...
(** **** Exercise: 3 stars, standard, especially useful (binary_commute)
    ...
    Recall the [incr] and [bin2nat] functions that you
    ...
    Name your theorem [bin2nat_pres_incr] ("pres" for "preserves").

    Before you start working on this exercise, copy the definitions of
    [incr] and [bin2nat] from your solution to the [binary]
...
(** Complete the definitions below of an increment function [incr]
    for binary numbers, and a function [bin2nat] to convert
...
Example test1 : nat2bin (S (S (S O))) = B1 (B1 Z).
Proof. simpl. reflexivity. Qed.

Example test2 : nat2bin (S (S (S (S O)))) = B0 (B0 (B1 Z)).
Proof. simpl. reflexivity. Qed.

Example test3 : nat2bin (S (S (S (S (S O))))) = B1 (B0 (B1 Z)).
Proof. simpl. reflexivity. Qed.

(** Prove that, if we start with any [nat], convert it to binary, and
    convert it back, we get the same [nat] we started with.  (Hint: If
    your definition of [nat2bin] involved any extra functions, you
    may need to prove a subsidiary lemma showing how such functions
    relate to [nat2bin].) *)

Theorem bin2nat_incr : forall b, bin2nat (incr b) = S (bin2nat b).
Proof.
  induction b as [ | |b IH].
  - reflexivity.
  - simpl. rewrite add_0_r. rewrite add_comm. reflexivity.
  - simpl. rewrite IH. rewrite 2 add_0_r.
    assert (H : bin2nat b + bin2nat b + 1 = S (bin2nat b + bin2nat b)).
    { rewrite add_comm. reflexivity. }
    rewrite H. simpl. rewrite add_comm. reflexivity.
Qed.

Theorem nat_bin_nat : forall n, bin2nat (nat2bin n) = n.
Proof.
  induction n as [|n IH].
  - reflexivity.
  - simpl. rewrite bin2nat_incr. rewrite IH. reflexivity.
Qed.
...
(** (c) Define a normalization function -- i.e., a function
        ...
        define this using [nat2bin] and [bin2nat]! *)
