...
Theorem app_nil_r : forall (X:Type), forall l:list X,
  l ++ [] = l.
Proof.
  intros X l. induction l. reflexivity. simpl. rewrite IHl. reflexivity.
Qed.
...
Definition partition {X : Type}
                     ...
                     (l : list X)
                   : list X * list X :=
  let a := filter test l in
  let b := filter (fun x => negb (test x)) l in
  (a, b).
...
Proof.
  ...
  - simpl. rewrite <- IHl. clear IHl.
    assert (H : forall m e, map f (m ++ [e]) = map f m ++ [f e]). {
      intros m e. induction m.
      - reflexivity.
...
Theorem fold_map_equals_map : forall X Y f l,
  @map X Y f l = fold_map f l.
Proof.
  intros X Y f l. induction l.
  - reflexivity.
  - simpl. rewrite IHl. reflexivity.
