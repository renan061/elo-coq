...
Fixpoint oddmembers (l:natlist) : natlist :=
  match l with
  | nil => nil
  | n :: l' => if even n then oddmembers l' else n :: oddmembers l'
  end.
...
Fixpoint count (v : nat) (s : bag) : nat :=
  match s with
  | nil => 0
  | x :: xs => let n := count v xs in if eqb x v then S n else n
  end.
...
Fixpoint member (v : nat) (s : bag) : bool :=
  match s with
  | nil => false
  | x :: xs => if eqb x v then true else member v xs
  end.
...
Fixpoint subset (s1 : bag) (s2 : bag) : bool :=
  ...
  | _, nil => false
  | nil, _ => true
  | x :: xs, _ => if member x s2 then subset xs (remove_one x s2) else false
  end.
...
Theorem add_inc_count : forall v bag n,
  count v bag = n -> count v (v :: bag) = n + 1.
Proof.
  intros v bag n H. simpl. rewrite eqb_refl. rewrite H.
...
Theorem app_nil_r : forall l : natlist,
  l ++ [] = l.
Proof.
  induction l as [| x xs IH]; simpl; try (rewrite IH); reflexivity.
Qed.

Theorem rev_app_distr: forall l1 l2 : natlist,
  rev (l1 ++ l2) = rev l2 ++ rev l1.
Proof.
  intros l1 l2. induction l1 as [| x xs IH].
...
Theorem rev_involutive : forall l : natlist,
  rev (rev l) = l.
Proof.
  induction l as [| x xs IH]; try reflexivity.
...
Proof.
  intros l1 l2 l3 l4. rewrite 2 app_assoc. reflexivity.
Qed.
...
Fixpoint eqblist (l1 l2 : natlist) : bool :=
  ...
  | _, nil => false
  | nil, _ => false
  | x :: xs, y :: ys => if eqb x y then eqblist xs ys else false
  end.
...
Lemma sum_empty_r : forall l, sum l [] = l.
Proof. destruct l; reflexivity. Qed.

Lemma sum_empty_l : forall l, sum [] l = l.
Proof. destruct l; reflexivity. Qed.

Lemma aux : forall x xs v,
  x = v -> count v (x :: xs) = S (count v xs).
Proof.
  intros x xs v H.
  rewrite H. simpl.
  rewrite eqb_refl.
...
Lemma count_empty : forall v,
  count v [ ] = 0.
Proof. intros v. reflexivity.
Qed.

(* TODO : could not find a solution. *)

Theorem bag_count_sum : forall (s1 s2 : bag) v,
  count v (sum s1 s2) = count v s1 + count v s2.
Proof.
  intros s1 s2 v.
  induction s1.
  try rewrite sum_empty_l.
  try rewrite sum_empty_r.
  try reflexivity.
  simpl.
  destruct (n =? v);
  simpl;
...
Proof.
  intros l n. destruct l; reflexivity.
Qed.
(** [] *)
...
Proof.
  intros m x y n H. simpl. rewrite H. reflexivity.
Qed.
End PartialMap.
