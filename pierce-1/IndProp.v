...
Proof.
  intros n m Hn Hm. induction Hn as [| n' En' IHn'].
  - simpl. apply Hm.
...
Proof.
  ...
  induction H2.
  - apply H1.
  - apply IHev. clear IHev. simpl in H1. apply evSS_ev in H1. apply H1.
Qed.
(** [] *)
...
Proof.
  ...
  apply ev_ev__ev.
  rewrite add_assoc.
  rewrite (add_comm (n + p) m).
  rewrite (add_assoc m n p).
  rewrite <- add_assoc.
  rewrite (add_comm m n).
  apply ev_sum.
  apply H.
  assert (p + p = double p) as H'.
  { induction p. reflexivity. simpl. f_equal. rewrite add_comm.
    rewrite PeanoNat.Nat.add_succ_l. f_equal. apply IHp. }
  rewrite H'.
  apply ev_double.
Qed.
(** [] *)
