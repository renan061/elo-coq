...
Inductive consistent_references (m : mem) : tm -> Prop :=
  ...
  | cr_acq   : forall t1 t2,  consistent_references m t1 ->
                              consistent_references m t2 ->
                              consistent_references m <{acq t1 t2    }>
...
Local Ltac _cr tt :=
  ...
  | H : consistent_references _ <{_ := _      }> |- _ => tt H
  | H : consistent_references _ <{acq _ _     }> |- _ => tt H
...
Lemma cr_write_address : forall m t1 t2 ad t,
  ...
  ad < #m.
Proof.
  intros. ind_tstep; repeat invc_cr; eauto;
...
Proof.
  intros.
  induction t; repeat invc_cr; eauto using consistent_references;
  simpl; destruct str_eq_dec; eauto using consistent_references.
Qed.
...
Proof.
  ...
  eauto; omicron; eauto; discriminate.
Qed.

Local Ltac solve_mem_set m ad :=
  intros ? t **;
  assert (ad < #m) by
    (decompose sum (lt_eq_lt_dec ad (#m)); subst; sigma; trivial; discriminate);
...
Local Ltac solve_mem_acq_rel :=
  ...
  (eapply cr_refR || eapply cr_refX || eapply cr_refW);
  sigma; eauto; repeat omicron; eauto.
...
Lemma cr_preservation_mem_alloc : forall m m' t1 t2 T,
  well_typed_term t1 ->
  consistent_references m t1 ->
  (* --- *)
  ...
  forall_memory m' (consistent_references m').
Proof.
  intros ** ? ? ?. subst. upsilon. eauto using cr_mem_add.
Qed.
...
Lemma cr_preservation_unt_alloc : forall m t1 t2 tu T,
  consistent_references m tu ->
  t1 --[e_alloc (#m) T]--> t2 ->
  consistent_references (m +++ (None, T, false)) tu.
Proof.
  intros. ind_tstep; eauto using cr_mem_add.
Qed.
...
Lemma cr_preservation_mem_acq : forall m t1 t2 ad t,
  ...
  forall_memory m (consistent_references m) ->
  t1 --[e_acq ad t]--> t2 ->
  forall_memory m[ad.X <- true] (consistent_references m[ad.X <- true]).
Proof.
  intros ** ? ? ?. omicron; eauto using cr_mem_acq.
Qed.
...
Lemma cr_preservation_unt_acq : forall m t1 t2 tu ad t,
  ad < #m ->
  consistent_references m tu ->
  t1 --[e_acq ad t]--> t2 ->
  consistent_references m[ad.X <- true] tu.
Proof.
  eauto using cr_mem_acq.
Qed.

(* rel --------------------------------------------------------------------- *)

Lemma cr_preservation_mem_rel : forall m t1 t2 ad,
  forall_memory m well_typed_term ->
  well_typed_term t1 ->
  consistent_references m t1 ->
  (* --- *)
  ...
  forall_memory m (consistent_references m) ->
  t1 --[e_rel ad]--> t2 ->
  forall_memory m[ad.X <- false] (consistent_references m[ad.X <- false]).
Proof.
  intros ** ? ? ?. omicron; eauto using cr_mem_rel.
Qed.
...
Lemma cr_preservation_unt_rel : forall m t1 t2 tu ad,
  ad < #m ->
  consistent_references m tu ->
  t1 --[e_rel ad]--> t2 ->
  consistent_references m[ad.X <- false] tu.
Proof.
  eauto using cr_mem_rel.
Qed.
...
Proof.
  ...
  - eauto using cr_preservation_mem_acq.
  - upsilon; eauto using cr_preservation_acq, cr_preservation_unt_acq.
  - eauto using cr_preservation_mem_rel.
  - upsilon; eauto using cr_preservation_rel, cr_preservation_unt_rel.
